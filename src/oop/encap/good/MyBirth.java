...
public class MyBirth {
    ...
    - 외부에서 직접 필드에 접근 할 수 없도록 필드에 private 제한을 붙힘
    */
    ...
    public void setDay(int day) {
        if(day < 1 || day > 31){
            ...
            if(this.month == 0){
                System.out.println("일자가 할당하기 위해서는 월 세팅이 선행 되어야 합니다.");
                return;
            }
        }
    ...
    // 메서드가 private 이다 -> 해당 클래스 안에서만 호출한다. 외부에서는 아예 접근
    //                        할수 없다
    ...
    public void birthInfo(){
        // year , month, day 셋 중 단 하나라도재댜로 값이 세팅 되지 않았다면
        // 출력 x
